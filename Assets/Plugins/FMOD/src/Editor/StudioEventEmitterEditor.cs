...
namespace FMODUnity
{
    ...
    public class StudioEventEmitterEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            //-- Google Resonance Occlusion start --//

            var occlusionEnabled = serializedObject.FindProperty("OcclusionEnabled");
            var occlusionParam = serializedObject.FindProperty("OcclusionParameterName");
            var occlusionIntensity = serializedObject.FindProperty("OcclusionIntensity");
            var currentOcclusion = serializedObject.FindProperty("CurrentOcclusion");
            var nextOcclusionUpdate = serializedObject.FindProperty("NextOcclusionUpdate");

            //-- Google Resonance Occlusion end --//
            ...
            if (editorEvent != null)
            {
                ...
                if(occlusionEnabled != null)
                {
                    occlusionEnabled.isExpanded = EditorGUILayout.Foldout(occlusionEnabled.isExpanded, "Occlusion Settings");
                    if (occlusionEnabled.isExpanded)
                    {
                        EditorGUILayout.PropertyField(occlusionEnabled, new GUIContent("Activate Resonance Occlusion"));
                        EditorGUILayout.PropertyField(occlusionParam, new GUIContent("Occlusion Parameter Name"));
